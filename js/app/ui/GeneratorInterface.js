...
import { el,on,attr } from '/js/lib/Html.js';
import { card,btn,items,switcher } from '/js/app/ui/InterfaceComponents.js';
        ...
        return this.#ui['card'] = card({
            ...
            this.#ui['parameters'] = el('div', { class: "d-none list-group list-group-flush" }, [
                items(this.#generator.consumption, {
                    label: "Consumption",
                    addCounter: ((el, id) => this.#ui[`consumption-${id}`] = el).bind(this),
                    addMeter: ((el, id) => this.#ui[`meter-consumption-${id}`] = el).bind(this),
                }),

                items(this.#generator.production, {
                    label: "Production",
                    meter: false,
                    addCounter: ((el, id) => this.#ui[`production-${id}`] = el).bind(this),
                    addMeter: ((el, id) => this.#ui[`meter-production-${id}`] = el).bind(this),
                }),

                items(this.#generator.upgradeCosts, {
                    label: "Upgrade Costs",
                    addCounter: ((el, id) => this.#ui[`upgrade-costs-${id}`] = el).bind(this),
                    addMeter: ((el, id) => this.#ui[`meter-upgrade-costs-${id}`] = el).bind(this),
                }),
                ...
                el('div', { class: "px-0 list-group-item" }, [
                    ...
                    el('input', {
                        type: "range", class: "d-block w-100 mb-2",
                    ...
                    switcher({
                        ...
                        addCheckbox: (checkbox => this.#ui['auto-produce-checkbox'] = checkbox).bind(this),
                    }),
                    ...
                    switcher({
                        ...
                        addCheckbox: (checkbox => this.#ui['auto-upgrade-checkbox'] = checkbox).bind(this),
                    }),
                ]),
            ]),
        ]);
